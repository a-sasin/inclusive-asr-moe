...
# Using FastConformer MoE pretrained on GRANARY (2026-03-18 run)
PRETRAINED_NEMO=${PRETRAINED_NEMO:-"/lp-dev/amelia/experiments/MoE/GRANARY_moe_fast_conformer_en/FastConformer-CTC-BPE/2026-03-18_11-42-22/checkpoints/FastConformer-CTC-BPE.nemo"}
...
EXPERIMENT_NAME="moe_finetune_librispeech_${TIMESTAMP}"
EXP_DIR=${PROJECT_ROOT}/inclusive-asr-moe/experiments/multilingual/finetune_librispeech/${EXPERIMENT_NAME}
LOG_DIR=${EXP_DIR}/logs

mkdir -p "${LOG_DIR}"

echo "========================================"
echo "MoE FastConformer → LibriSpeech Fine-tuning"
echo "========================================"
echo "Experiment:  ${EXPERIMENT_NAME}"
echo "Pretrained:  ${PRETRAINED_NEMO}"
echo "Exp dir:     ${EXP_DIR}"
echo "GPUs:        ${CUDA_VISIBLE_DEVICES}"
echo "NOTE:        load_balance_loss_weight = 0.002 (enabled for auxiliary loss during fine-tuning)"
echo ""
...
if [ ! -f "${PRETRAINED_NEMO}" ]; then
    ...
    echo "       - /lp-dev/amelia/experiments/MoE/GRANARY_moe_fast_conformer_en/FastConformer-CTC-BPE/2026-03-18_11-42-22/checkpoints/FastConformer-CTC-BPE.nemo"
    exit 1
fi
...
############################################
{
  echo "=== Fine-tune: MoE FastConformer → LibriSpeech ==="
  echo "date: $(date -Is)"
  echo "hostname: $(hostname)"
  echo "CUDA_VISIBLE_DEVICES: ${CUDA_VISIBLE_DEVICES}"
  echo "pretrained_nemo: ${PRETRAINED_NEMO}"
  echo "config: ${CONFIG_FILE}"
  echo "load_balance_loss_weight: 0.002 (enabled for auxiliary loss)"
