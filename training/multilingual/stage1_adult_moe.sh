...
CONFIG_PATH=${PROJECT_DIR}/configs/multilingual
CONFIG_NAME=adult_moe.yaml
TOKENIZER_DIR=${PROJECT_DIR}/tokenizers/granary_multilingual_bpe_16384/tokenizer_spe_bpe_v16384


PRETRAINED_NEMO=${PRETRAINED_NEMO:-"${PROJECT_DIR}/baseline_weights/stt_ml_fastconformer_ctc_large_to_moe_init.nemo"}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"moe_fastconformer_multilingual_${TIMESTAMP}"}
EXPERIMENT_ROOT=${PROJECT_DIR}/experiments/multilingual/adult/moe
...
echo "========================================"
echo "MoE FastConformer -> Multilingual fine-tuning"
echo "========================================"
...
############################################
{
    echo "=== Fine-tune: MoE FastConformer -> Multilingual ==="
  echo "date: $(date -Is)"
  ...
  echo "git (project): $(cd "${PROJECT_DIR}" && git rev-parse HEAD 2>/dev/null || echo 'n/a')"
